...
def QrCodeVerificator(request):
    try:
        ...
        if QrDataDecrypted == user.QrUid:
            # If the user is invited to the event, it returns the user data,
            #  a check value of true and an invited value of true to the frontend.
            if guestcheck is True:
                check = "true"
                notjsondata = {
                    "userdata": {
            ...
            # If the user is not invited to the event, it returns a invited value of false.
                ...
                check = "true"
                notjsondata = {"check": check, "invited": "false"}
        # If the qr data does not match the user in the database, it returns a check value of false.
        else:
            check = "false"
            notjsondata = {"check": check, "user not found": "User not Found"}
        ...
        check = "false"
        notjsondata2 = {"check": check, "Fernet error": "Fernet error"}
